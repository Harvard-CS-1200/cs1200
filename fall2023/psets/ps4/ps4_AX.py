...
#     above = [] # 
#     equal = []
#     # k = get_random_index(arr)
#     k = arr[get_random_index(arr)][0]
...
#     elif i >= len(below) + len(equal):
#         return QuickSelect(above, i - len(below) - len(equal))
#     else:
#         return equal[0] 
...
def QuickSelect(arr, i):
    ...
    a, b, c = get_random_index(arr), get_random_index(arr), get_random_index(arr)
    med = [arr[a][0], arr[b][0], arr[c][0]]
    med.sort()
    k = med[1]

    # three possibilities for median
...
def experiments():
    # Edit this parameter
    k = [17, 19, 21, 22, 23, 24, 25, 26]

    # Feel free to edit these initial parameters

    RUNS = 300 # Number of runs for each trial; more runs means better distributions approximation but longer experiment
